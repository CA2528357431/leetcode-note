...
## 496. 跳跃游戏 IV
* 给你一个数组`arr`，你一开始在第一个元素处。你可以从下标`i`跳到：`i + 1`--满足：`i + 1 < arr.length`；`i - 1`--满足：`i - 1 >= 0`；`j`--满足：`arr[i] == arr[j]`且`i != j`。 请你返回到达最后一个元素的下标处所需的最少操作次数

## 497. 删除回文子序列
* 给你一个字符串`s`，它仅由字母`'a'`和`'b'`组成。每一次删除操作都可以从`s`中删除一个回文子序列。 返回删除给定字符串中所有字符的最小删除次数

## 498. 打折购买糖果的最小开销
* 每购买两个糖果，商店会送一个糖果。 送糖果的限制是：它的价格需要小于等于购买的两个糖果价格的较小值 。给一个下整数数组`cost`，其中`cost[i]`表示第`i`个糖果的价格。请返回获得所有糖果的最小总开销

## 499. 统计隐藏数组数目
* 给你一个长度为 n 的整数数组`differences`，其是`hiden`数组的描述，`differences[i] = hidden[i + 1] - hidden[i]`。隐藏数组中所有数字的值都在闭区间`[lower, upper]`中。请你返回符合要求的隐藏数组的数目

## 500. 价格范围内最高排名的 K 样物品
* 给你一个二维整数数组`grid`，数组中的整数含义为：`0`表示无法穿越的一堵墙；`1`表示可以自由通过的一个空格子； 所有其他正整数表示该格子内的一样物品的价格。给整数数组`pricing`，其中`pricing = [low, high]`表示你只对物品价格在闭区`[low, high]`之内的物品感兴趣。 给一个数组`start`表示开始位置。给你一个整数`k`。 物品排名按照优先级从高到低的以下规则： 到`start`的距离近；价格低；行坐标小；列坐标小。 请你返回排名最高的`k`件物品的坐标，按照排名排序后返回。如果不足`k`件，则全部返回。
